        ...
        <div class="flex items-center justify-between py-6 lg:justify-start lg:space-x-10">
            ...
            <nav class="hidden flex-1 justify-center space-x-10 lg:flex">
                @foreach ($menu as $item)
                    <a href="{{ $item['href'] }}" class="text-lg text-white hover:text-opacity-90 transition-all">
                        {{ $item['title'] }}
                    </a>
                @endforeach
            </nav>
                ...
                <div class="px-4 pb-6 pt-4">
                    ...
                    <div class="mt-6">
                        <nav class="grid gap-y-2">
                            @foreach ($menu as $item)
                                <a href="{{ $item['href'] }}" class="flex items-center rounded-md bg-gray-100 p-3 hover:bg-gray-50">
                                    <x-icon :name="$item['icon']" class="h-6 w-6 text-secondary-100" />
                        ...
                        <div class="mt-8 flex justify-center">
                            <a href="https://demo.rapidez.io" target="_blank" class="inline-flex items-center justify-center whitespace-nowrap rounded-full border border-transparent bg-gradient-to-b from-primary-100 to-primary-200 px-6 py-3 text-base font-bold text-white opacity-100 transition duration-150 ease-in-out hover:opacity-70">
                                Demo
                            </a>
                        </div>
                    </div>
